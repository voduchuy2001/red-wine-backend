exports.MESSAGES = {
    ...
    isLength: (min, max) =>
        `${
            min && max
                ? `From ${min} - ${max} character(s)`
                : min
                  ? `Greater than ${min} character(s)`
                  : max
                    ? `Less than ${max} character(s)`
                    : ""
        }`,
    ...
    tooManyRequests: (minutes) => `Too many requests. Please try again after ${minutes} minute(s)`,
